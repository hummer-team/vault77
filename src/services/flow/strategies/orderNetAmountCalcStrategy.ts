  ...
  async postProcess(
    ...
    const enrichedRows = rows.map((row) => {
      ...
      return {
        ...
        refund_risk_label: refundRiskTag === 'high' ? '高' : refundRiskTag === 'medium' ? '中' : '低',
        is_valid: isValid,
        is_valid_label: isValid ? '有效' : '无效',
    ...
    const displayConfig: OperatorDisplayConfig = {
      ...
      columnTooltips: {
        pay_amount: '订单实付金额',
        refund_amount: '订单退款金额',
        rejection_amount: '订单拒签金额',
        order_status: '订单状态原始值（如 PAID/CANCELLED/REFUNDED 等）',
        net_amount: '订单净额 = 实付 - 退款 - 拒签（取消订单为0）',
        net_amount_raw: '订单净额（原始计算值，未取整）',
        net_amount_rounded: '净额保留2位小数',
        refund_rate: '退款率 = 退款金额 / 实付金额',
        refund_rate_raw: '退款率（原始计算值，退款金额/实付金额）',
        refund_rate_percent: '退款率百分比',
        refund_risk_tag: '退款风险等级：high/medium/low',
        refund_risk_label: '退款风险等级（高/中/低），基于退款率划分',
        is_valid: '是否为有效订单（非排除状态）',
        is_valid_label: '是否为有效订单（非排除状态）',
